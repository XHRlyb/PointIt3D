...
def main():
	...
	parser.add_argument("--data_dir", default='ScanNet', type=str, help="path to ScanNet folder")
	...
	with open(eric_file, 'rb') as f:
		...
		fac = []
		xxxx = 0.016193
		yyyy = -0.053282  # arm's vector
		costhe = ((xxxx * cx + yyyy * cy) / (math.sqrt(xxxx * xxxx + yyyy * yyyy) * math.sqrt(cx * cx + cy * cy)))
		sinthe = ((xxxx * cy - yyyy * cx) / (math.sqrt(xxxx * xxxx + yyyy * yyyy) * math.sqrt(cx * cx + cy * cy)))
		print(costhe, sinthe)
